...
-- FORALLEXPR  ::=  Forall VAR (EXPR)
forAllExpr = do {char 'F'; char 'o'; char 'r'; char 'a'; char 'l'; char 'l'
                ;whiteSpace
                ;(Var n) <- var
                ;whiteSpace
                ;retExpr <- (parens expr) 
...
-- EXISTSEXPR  ::=  Exists VAR (EXPR)
existExpr  = do {char 'E'; char 'x'; char 'i'; char 's'; char 't'; char 's'
                ;whiteSpace
                ;(Var n) <- var
                ;whiteSpace
                ;retExpr <- (parens expr) 
...
-- NEGEXPR     ::=  ~ATOM
negExpr    = do {reservedOp "~"
                ;retNegExpr <- atom
                ;return (Not retNegExpr)
                }

-- TERM        ::=  FACTOR { & FACTOR }^*
term       = do {retTerm <- sepBy1 factor (reservedOp "&") 
                ;return retTerm
                }

-- FACTOR      ::=  NEGEXPR | FORALLEXPR | EXISTSEXPR | ATOM
factor     = do {retFact <- (negExpr <|> forAllExpr <|> existExpr <|> atom)
                ;return retFact
                }
