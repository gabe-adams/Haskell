...
smash :: String -> String
smash str = [toLower c | c <- str, c `elem` ['a'..'z'] || c`elem`['A'..'Z']]
...
shift n str = [ toChar ((n + toNum c )`mod`26) | c<- smash str]
...
capitalized "" = ""
capitalized (c:cs) = (toUpper c) : [toLower m | m<-cs]
...
title :: [String] -> [String]
title [] = []
title (first:rest) = capitalized first : [if (length a) > 4 then capitalized a else [toLower b| b<-a] | a<-rest]
