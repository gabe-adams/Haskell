...
data OpCode = HALT 
            ...
            | IADD | ISUB | IMULT | ILT   | INC   | DEC  | NI
            | JMP  | JZ   | JNZ
            | CALL | RET
...
[halt,push,pop,dup,fetch,store,
     iadd,isub,imult,ilt,inc,dec,ni,jmp,jz,jnz,call,ret] 
...
-- For Homework 7
        NI    -> (pc+1, sp, set stk (sp-1) b             , regs)
        	where b = boolToWrd (top==0)
        JMP   -> (pc' , sp  , stk                        , regs)
            where pc' = pc+arg+1
        JZ    -> (pc' , sp-1, stk                        , regs)
